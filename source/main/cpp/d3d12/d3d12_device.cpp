...
extern "C"
{
    _declspec(dllexport) extern const UINT D3D12SDKVersion = 711;
}
extern "C"
{
...
namespace ncore
{
    namespace ngfx
    {
        ...
        static void __stdcall D3D12MessageCallback(D3D12_MESSAGE_CATEGORY Category, D3D12_MESSAGE_SEVERITY Severity, D3D12_MESSAGE_ID ID, LPCSTR pDescription, void* pContext) { RE_DEBUG(pDescription); }

        static IDXGIAdapter1* FindAdapter(IDXGIFactory6* pDXGIFactory, D3D_FEATURE_LEVEL minimumFeatureLevel)
        {
            IDXGIAdapter1* adapters[64];
            s32            num_adapters = 0;

            RE_DEBUG("available GPUs :");
            IDXGIAdapter1* pDXGIAdapter = nullptr;
            for (UINT adapterIndex = 0; DXGI_ERROR_NOT_FOUND != pDXGIFactory->EnumAdapterByGpuPreference(adapterIndex, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&pDXGIAdapter)); ++adapterIndex)
            {
                DXGI_ADAPTER_DESC1 desc;
                pDXGIAdapter->GetDesc1(&desc);
                RE_DEBUG("  - {}", wstring_to_string(desc.Description));
                adapters[num_adapters++] = pDXGIAdapter;
            }

            s32 selectedAdapter = -1;
            for (s32 i = 0; i < num_adapters; ++i)
            {
                DXGI_ADAPTER_DESC1 desc;
                ...
                if (!isSoftwareGPU && capableFeatureLevel)
                {
                    selectedAdapter = i;
                    break;
                }
            ...
            for (s32 i = 0; i < num_adapters; ++i)
            {
                if (selectedAdapter == -1 || i != selectedAdapter)
                {
                    adapter->Release();
                }
            }
        ...
        IGfxShader* D3D12Device::CreateShader(const GfxShaderDesc& desc, eastl::span<u8> data, const eastl::string& name)
        {
        ...
        bool D3D12Device::Create()
        {
            ...
            bool capableDevice = m_featureSupport.HighestShaderModel() >= D3D_SHADER_MODEL_6_6 && m_featureSupport.ResourceBindingTier() >= D3D12_RESOURCE_BINDING_TIER_3 && m_featureSupport.RaytracingTier() >= D3D12_RAYTRACING_TIER_1_1 &&
                                 m_featureSupport.RenderPassesTier() >= D3D12_RENDER_PASS_TIER_0 && m_featureSupport.MeshShaderTier() >= D3D12_MESH_SHADER_TIER_1 && m_featureSupport.WaveOps() && m_featureSupport.Native16BitShaderOpsSupported() &&
        ...
        D3D12Descriptor D3D12Device::AllocateRTV() { return m_pRTVAllocator->Allocate(); }

        D3D12Descriptor D3D12Device::AllocateDSV() { return m_pDSVAllocator->Allocate(); }

        D3D12Descriptor D3D12Device::AllocateResourceDescriptor() { return m_pResDescriptorAllocator->Allocate(); }

        D3D12Descriptor D3D12Device::AllocateSampler() { return m_pSamplerAllocator->Allocate(); }

        D3D12Descriptor D3D12Device::AllocateNonShaderVisibleUAV() { return m_pNonShaderVisibleUavAllocator->Allocate(); }
        ...
        void D3D12Device::CreateRootSignature()
        {
            ...
            for (u32 i = 1; i < GFX_MAX_CBV_BINDINGS; ++i)
            {
        ...
        void D3D12Device::CreateIndirectCommandSignatures()
        {
            ...
            desc.ByteStride         = sizeof(u32) + sizeof(D3D12_DRAW_ARGUMENTS);
            ...
            desc.ByteStride         = sizeof(u32) + sizeof(D3D12_DRAW_INDEXED_ARGUMENTS);
            ...
            desc.ByteStride         = sizeof(u32) + sizeof(D3D12_DISPATCH_ARGUMENTS);
            ...
            desc.ByteStride         = sizeof(u32) + sizeof(D3D12_DISPATCH_MESH_ARGUMENTS);
        ...
        D3D12DescriptorAllocator::D3D12DescriptorAllocator(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE type, bool shader_visible, u32 descriptor_count, const eastl::string& name)
        {
        ...
        D3D12Descriptor D3D12DescriptorAllocator::Allocate()
        {
            ...
            ASSERT(m_allocatedCount < m_descirptorCount);
        ...
        void D3D12DescriptorAllocator::Free(const D3D12Descriptor& descriptor) { m_freeDescriptors.push_back(descriptor); }

        D3D12Descriptor D3D12DescriptorAllocator::GetDescriptor(u32 index) const
        {
        ...
        D3D12ConstantBufferAllocator::D3D12ConstantBufferAllocator(D3D12Device* device, u32 buffer_size, const eastl::string& name)
        {
            GfxBufferDesc desc;
        ...
        void D3D12ConstantBufferAllocator::Allocate(u32 size, void** cpu_address, u64* gpu_address)
        {
            ASSERT(m_allocatedSize + size <= m_pBuffer->GetDesc().size);
        ...
        void D3D12ConstantBufferAllocator::Reset() { m_allocatedSize = 0; }
