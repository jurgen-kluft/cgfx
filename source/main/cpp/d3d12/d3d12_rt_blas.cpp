...
namespace ncore
{
    namespace ngfx
    {
        D3D12RayTracingBLAS::D3D12RayTracingBLAS(D3D12Device* pDevice, const GfxRayTracing::BLASDesc& desc, const char* name)
        {
            m_pDevice = pDevice;
            m_desc    = desc;
            m_name    = name;
        ...
        bool D3D12RayTracingBLAS::Create()
        {
            ...
            {
                const GfxRayTracing::Geometry& geometry = m_desc.geometries[i];
            ...
            CD3DX12_RESOURCE_DESC scratchBufferDesc = CD3DX12_RESOURCE_DESC::Buffer(math::max(info.ScratchDataSizeInBytes, info.UpdateScratchDataSizeInBytes), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
